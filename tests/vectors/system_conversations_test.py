...
class HorizontalConversationTest(unittest.TestCase):
    ...
    @for_range('h', -180, 180)  # перебор по парам a h и вызывов ф-и тестирующей
    @for_range('a', -90, 90)
    def qtest_save_radius_as_length(self, h, a): #запускаем функцию от a,h
        ...
        v = Vector.from_horizontal(*hor) #*hor - в функцию передали весь кортеж из трех аргументов. Переводим в декартову
        self.assertLess(abs(1000 - v.length), self.epsilon, str(h) + ' ' + str(a)) #проверяем, что радиус в полярной = длина в декартовой
    ...
    def qtest_save_length_as_radius(self, h, a): #запускаем функцию от a,h
        ...
        self.assertLess(abs(v.length - hor2.r), self.epsilon, str(h) + ' ' + str(a)) #проверяем что длина в декартовой = радиус в полярной

    @for_range('h', -100, 100) # перебор по парам a h и вызывов ф-и тестирующей
    @for_range('a', -80, 80)
    def test_correct_reconversion(self, h, a):
        ...
        self.assertLess(self.distance(hor2, hor), self.epsilon, str(hor) + '->' + str(hor2)) #проверяем, что расст -> 0
